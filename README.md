...
| `log_log_holevo_information_plot.ipynb` | Python | Loads the `.pkl` data and produces the log–log plot of the gap $1-r$ (with $r$ the ratio of the Holevo information to the upper bound) versus $n$. This is the figure referenced in the paper; the gap scales like $c(d)\,n^{-\alpha}$ with $\alpha\approx 0.3$ roughly independent of $d$. |
